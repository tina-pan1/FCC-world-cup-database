...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE round = 'Eighth-Final' AND year
= 2014 ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id OR games.opponent_id = teams.team_id WHERE name LIKE 'Co%'")"
